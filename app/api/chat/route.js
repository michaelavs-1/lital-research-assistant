...
export async function POST(req) {
  ...
  const { messages = [], mode = "write", lang = "he", apiKey: userKey } = body;
  const apiKey = (userKey && userKey.trim()) || process.env.OPENAI_API_KEY;
  if (!apiKey) {
    return new Response(
      "לא הוגדר מפתח OpenAI. לחצי על כפתור המפתח בפינה והדביקי אותו.",
      { status: 400 }
    );
  }
